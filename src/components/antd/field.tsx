...
function FieldGroup({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="field-group" className={cn("group/field-group flex w-full flex-col gap-7 data-[slot=checkbox-group]:gap-3", className)} {...props} />;
}

function Field({ className, orientation = "vertical", ...props }: React.ComponentProps<"div"> & { orientation?: "vertical" | "horizontal" | "responsive" }) {
  return <div role="group" data-slot="field" data-orientation={orientation} className={cn("group/field flex w-full gap-3 data-[invalid=true]:text-destructive", orientation === "vertical" && "flex-col [&>*]:w-full", orientation === "horizontal" && "flex-row items-center", orientation === "responsive" && "flex-col md:flex-row md:items-center", className)} {...props} />;
}
...
function FieldLabel({ className, ...props }: React.ComponentProps<typeof Label>) {
  return <Label data-slot="field-label" className={cn("group/field-label flex w-fit gap-2 leading-snug", className)} {...props} />;
}
