...
import { startExamAttemptApi, updateExamAttemptProgressApi, type Question } from "@/apis/examAttemptApi";
import { env } from "@/lib/env";
import {
  fetchAndDecryptSecureQuiz,
  submitSecureQuiz,
  type DecryptedQuestion,
  type QuestionTokenMap,
} from "@/lib/secureQuizHelpers";

export default function ExamQuizPage() {
  const { id: examSessionId = "" } = useParams();
  ...
  const [markedForReview, setMarkedForReview] = useState<Set<string>>(() => new Set<string>());
  ...
  useEffect(() => {
    if (!fullscreenConfirmed || !examSessionId) return;
    ...
    const initExam = async () => {
      ...
      try {
        const startResult = await startExamAttemptApi(examSessionId, captchaToken || undefined);
        setAttemptId(startResult.examAttempt.id);

        const secureQuizData = await fetchAndDecryptSecureQuiz(startResult.examAttempt.id);
        ...
        const message = err?.response?.data?.message || err?.message || "Không thể bắt đầu bài thi";
        setError(message);
        toast.error(message);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    return () => {
      ...
      if (cleanupFns) {
        ...
        document.removeEventListener("contextmenu", cleanupFns.preventRightClick);
      }
  ...
  }, [timeSpentSeconds, timeLimitMinutes, loading, isSubmitted, hasWarned30s, handleAutoSubmit]);
  ...
  useEffect(() => {
    const handleBeforeUnload = () => {
      if (attemptId && !isSubmitted && env.apiUrl) {
        navigator.sendBeacon(
          `${env.apiUrl}/examattempts/${attemptId}/progress`,
          JSON.stringify({
            answers,
            currentIndex: currentQuestionIndex,
            markedQuestions: Array.from(markedForReview),
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      const message = err?.response?.data?.message || err?.message || "Nộp bài thất bại";
      toast.error(message);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Button onClick={() => navigate(`/exam-session/${examSessionId}`)}>Quay lại</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-4">Bài thi này chưa có câu hỏi nào</p>
      ...
      return (
        <div className="min-h-screen">
          <div className="container max-w-7xl mx-auto py-6 px-4">
            <div className="mb-6">
              <Button variant="ghost" onClick={() => setShowDetailedResults(false)} className="mb-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              <div className="lg:col-span-3 space-y-6">
                <Card className={cn("border-2", isCorrect ? "border-green-500" : "border-destructive")}>
                  <CardContent className="pt-6">
                    ...
                    <div className="prose max-w-none mb-6" dangerouslySetInnerHTML={{ __html: reviewQuestion.question }} />

                    <div className="space-y-3">
                      {reviewQuestion.options.map((option, index) => {
                        const optionLetter = String.fromCharCode(65 + index);
                        const isUserAnswer = userAnswer === optionLetter;
                        const isCorrectAnswer = reviewQuestion.answer === optionLetter;
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              isUserAnswer && !isCorrectAnswer && "border-destructive bg-red-50",
                          ...
                          >
                            ...
                            <div className="flex-1">
                              ...
                              {isCorrectAnswer && <span className="ml-2 text-green-600 text-sm font-medium">(Đáp án đúng)</span>}
                              {isUserAnswer && !isCorrectAnswer && <span className="ml-2 text-destructive text-sm font-medium">(Bạn chọn)</span>}
                            </div>
                          </div>
                        );
                      })}
                    ...
                    {!userAnswer && (
                      <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-yellow-800 text-sm">Bạn không trả lời câu hỏi này</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
                ...
                <div className="flex justify-between">
                  <Button variant="outline" onClick={() => setReviewQuestionIndex((prev) => prev - 1)} disabled={reviewQuestionIndex === 0}>
                    <ChevronLeft className="h-4 w-4 mr-2" />
                    Câu trước
                  </Button>
                  ...
                  <CardContent className="pt-6">
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              !qIsCorrect && qUserAnswer && "bg-destructive text-white",
                    ...
                    <div className="mt-6 space-y-2 text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>Sai ({resultData.questions.filter((q) => q.userAnswer && q.userAnswer !== q.answer).length})</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="w-4 h-4 bg-yellow-500 rounded" />
                        <span>Chưa trả lời ({resultData.questions.filter((q) => !q.userAnswer).length})</span>
                      </div>
                    </div>

                    <div className="mt-6 space-y-2">
                      <Button className="w-full" variant="outline" onClick={() => setShowDetailedResults(false)}>
                        Xem tổng quan
                      </Button>
          ...
          <CardContent className="pt-6">
            <div className="text-center space-y-6">
              <div className="flex justify-center">
                {passed ? <CheckCircle2 className="h-20 w-20 text-green-500" /> : <XCircle className="h-20 w-20 text-destructive" />}
              </div>

              <div>
                <h1 className="text-3xl font-bold mb-2">{passed ? "Chúc mừng!" : "Chưa đạt"}</h1>
                <p className="text-muted-foreground">{passed ? "Bạn đã hoàn thành bài thi" : "Hãy cố gắng hơn lần sau"}</p>
              </div>
              ...
              <div className="grid grid-cols-5 gap-4 max-w-2xl mx-auto">
                ...
                <div className="text-center p-4 bg-muted rounded-lg">
                  <div className="text-2xl font-bold text-destructive">{totalQuestions - correctAnswersCount}</div>
                ...
                <div className="text-center p-4 bg-muted rounded-lg">
                  <div className="text-2xl font-bold">{formatTime(timeSpentSeconds)}</div>
                  <div className="text-sm text-muted-foreground">Thời gian</div>
                </div>
                <div className="text-center p-4 bg-muted rounded-lg">
                  <div className={`text-2xl font-bold ${cheatingDetection.totalViolations > 0 ? "text-orange-500" : "text-green-600"}`}>
              ...
              <div className="flex gap-4 justify-center flex-wrap">
                {resultData.showAnswers && resultData.questions && (
                  <Button onClick={() => { setShowDetailedResults(true); setReviewQuestionIndex(0); }} variant="default">
                    <BarChart3 className="h-4 w-4 mr-2" />
                    Xem chi tiết
                  </Button>
                )}
            ...
            <div className="flex justify-between items-start">
              ...
              <div
                className={cn(
                  ...
                  remainingTime !== null && remainingTime <= 60 ? "bg-destructive/10 text-destructive animate-pulse" : "bg-muted",
                )}
              >
                ...
                {remainingTime !== null ? (
                  <span>
                    ...
                    <span className="text-sm text-muted-foreground ml-1">còn lại</span>
                  </span>
                ) : (
            ...
            <div className="lg:col-span-3 space-y-6">
              {currentQuestion && (
                <Card>
                  <CardContent className="pt-6">
                    ...
                    <StandardMarkdownRenderer content={currentQuestion.question} className="mb-6" fontSize={16} />
                        ...
                        return (
                          ...
                          >
                            ...
                            <Label htmlFor={`option-${index}`} className="flex-1 cursor-pointer">
                              <StandardMarkdownRenderer content={option} fontSize={15} />
                            </Label>
                          </div>
                        );
                      })}
              ...
              <div className="flex justify-between">
                ...
                {currentQuestionIndex === totalQuestions - 1 ? (
                  <Button onClick={() => (answeredCount < totalQuestions ? setShowSubmitConfirm(true) : doSubmit())}>Nộp bài</Button>
                ) : (
            ...
            <div className="lg:col-span-1">
              <Card className="sticky top-6">
                <CardContent className="pt-6">
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {questions.map((q, index) => {
                      ...
                      return (
                        ...
                        >
                          {index + 1}
                          {isMarked && <Flag className="h-3 w-3 absolute top-0 right-0 text-orange-500" />}
                        </button>
                      );
                    })}
                  </div>
                  ...
                  {remainingTime !== null && remainingTime <= 300 && remainingTime > 0 && (
                    <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  <Button className="w-full mt-6" onClick={() => setShowSubmitConfirm(true)} variant="default">
                    Nộp bài
                  </Button>
                </CardContent>
              </Card>
