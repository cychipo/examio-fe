    ...
    <div className="min-h-screen space-y-8 bg-background p-6 pb-20">
      <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-muted-foreground">Thống kê hiệu quả giảng dạy và tài liệu học tập của bạn.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard title="Tổng số Đề thi" value={stats.summary.totalQuizSets} icon={BookOpen} iconColor="text-primary" iconBgColor="bg-red-100" description="Đã được tạo trong hệ thống" />
        <StatCard title="Tổng số Flashcard" value={stats.summary.totalFlashcardSets} icon={Layers} iconColor="text-green-600" iconBgColor="bg-green-100" description="Các bộ thẻ ghi nhớ đang hoạt động" />
        <StatCard title="Phòng thi đang mở" value={stats.summary.totalExamRooms} icon={Users} iconColor="text-yellow-600" iconBgColor="bg-yellow-100" description="Số lượng phòng thi của bạn" />
      </div>
          ...
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription>Số lượng đề thi và flashcard tạo trong {range === "7d" ? "7 ngày qua" : "30 ngày qua"}</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
              ...
              {isMounted && (
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                  <AreaChart data={creationChartData}>
                    ...
                    <Area type="monotone" dataKey="quizSets" name="Đề thi" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorQuiz)" />
                    <Area type="monotone" dataKey="flashcardSets" name="Flashcard" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorFlash)" />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription>Số lượt thi trong {range === "7d" ? "7 ngày qua" : "30 ngày qua"}</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
              ...
              {isMounted && (
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                  <AreaChart data={activityChartData}>
                    ...
                    <Tooltip contentStyle={{ borderRadius: "8px", border: "none", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }} />
                    <Legend iconType="circle" />
                    <Area type="monotone" dataKey="examAttempts" name="Lượt thi" stroke="#f59e0b" strokeWidth={3} fillOpacity={1} fill="url(#colorExam)" />
                    <Area type="monotone" dataKey="flashcardViews" name="Lượt học thẻ" stroke="#8b5cf6" strokeWidth={3} fillOpacity={1} fill="url(#colorFlashcard)" />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
            ...
            <div className="space-y-4">
              {stats.topRoomsByParticipants.length > 0 ? (
                stats.topRoomsByParticipants.map((room, index) => (
                  <div key={room.id} className="group flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="line-clamp-1 text-sm font-medium">{room.title}</span>
                    </div>
                ...
                <p className="py-10 text-center text-sm text-muted-foreground">Chưa có dữ liệu phòng thi</p>
              )}
            </div>
          </CardContent>
        </Card>
              ...
              {stats.topRoomsByAvgScore.length > 0 ? (
                stats.topRoomsByAvgScore.map((room, index) => (
                  <div key={room.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-500/10 text-sm font-bold text-amber-500">{index + 1}</div>
                      <span className="line-clamp-1 text-sm font-medium">{room.title}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-sm font-bold text-yellow-600">{room.avgScore}</span>
              ...
              stats.topFlashcardSets.map((set, index) => (
                ...
                >
                  ...
                  <div className="mb-2 flex items-center gap-2">
                    <span className="rounded bg-yellow-500/20 px-2 py-0.5 text-xs font-bold text-yellow-600">#{index + 1}</span>
                  </div>
                  <h3 className="mb-2 line-clamp-2 h-10 text-sm font-semibold">{set.title}</h3>
